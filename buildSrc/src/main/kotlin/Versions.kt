@file:Suppress("unused")

object SdkVersions {
    const val COMPILE_SDK = 29
    const val TARGET_SDK = 29
    const val MIN_SDK = 21
}

object BuildScriptVersions {
    const val BUILD_TOOLS_GRADLE = "4.1.0-beta05"
    const val DEXCOUNT_GRADLE = "1.0.3"
    const val VERSIONS_GRADLE = "0.29.0"
}

object Versions {
    const val KOTLIN = "1.3.72"
    const val KOTLIN_COROUTINES = "1.3.8"
    const val CORE = "1.3.1"
    const val FRAGMENT = "1.2.2"
    const val APPCOMPAT = "1.1.0"
    const val RECYCLERVIEW = "1.1.0"
    const val MATERIAL = "1.2.0-rc01"
    const val CONSTRAINT_LAYOUT = "2.0.0-beta8"
    const val PREFERENCE = "1.0.0"
    const val LIFECYCLE = "2.2.0"
    const val MOSBY = "2.0.1"
    const val BUTTERKNIFE = "10.2.1"
    const val TIMBER = "4.7.1"
    const val GSON = "2.8.2"
    const val RX_JAVA = "1.2.0"
    const val RX_ANDROID = "1.2.1"
    const val RX_PREFERENCES = "1.0.2"
    const val MATERIAL_DIALOGS = "0.9.6.0"
    const val SIMPLE_ITEM_DECORATION = "1.0.0"
    const val ABOUT_PAGE = "1.2.4"
}

object DebugVersions {
    const val STETHO = "1.5.1"
    const val DEBUGDRAWER = "0.8.0"
    const val SCALPEL = "1.1.2"
}
